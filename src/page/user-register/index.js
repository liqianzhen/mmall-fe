...
var page = {
    ...
    formValidate : function(formData){
        ...
        if(formData.password.length < 6){
            result.msg = 'password length should not be less than 6 digits';
            return result;
        }
        // 验证两次输入的密码是否一致
        if(formData.password !== formData.passwordConfirm){
            result.msg = 'The passwords you entered must be the same.';
            return result;
        }
        // 验证手机号
        ...
        if(!_mm.validate(formData.question, 'require')){
            result.msg = 'security problem should not be empty';
            return result;
        }
        // 验证密码提示问题答案是否为空
